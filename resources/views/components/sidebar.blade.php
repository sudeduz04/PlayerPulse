<aside class="w-64 bg-surface-800 border-r border-border flex flex-col min-h-screen shrink-0">
    {{-- Logo --}}
    <div class="p-5 border-b border-border">
        <div class="flex items-center gap-3">
            ...
            <div>
                <span class="text-accent font-bold text-sm">Akıllı Futbolcu Analizi</span>
                <p class="text-[10px] text-gray-500 uppercase tracking-wider">{{ $subtitle }}</p>
            </div>
        </div>
    </div>
        ...
        @foreach($navItems as $item)
            @if(isset($item['section']))
                <div class="pt-4 pb-2 px-3">
                    <span class="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">{{ $item['section'] }}</span>
                </div>
                @continue
            @endif
            ...
            <a href="{{ $item['url'] }}"
               @class([
                   ...
                   'bg-surface-700 text-accent border-l-2 border-accent font-medium' => $isActive,
                   'text-gray-400 hover:text-white hover:bg-surface-700' => !$isActive && !$isDisabled,
                   'text-gray-600 cursor-default' => $isDisabled,
               ])>
                {{-- Icons --}}
                @switch($item['icon'])
                    @case('dashboard')
                    ...
                    @case('clipboard')
                        ...
                        @break
                    @case('dumbbell')
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12h1m16 0h1M5.6 12H4m16.4 0H19M6.6 7v10M17.4 7v10M4 9.4v5.2M20 9.4v5.2M8.6 8v8h6.8V8H8.6z"/></svg>
                        @break
                    @case('chart-bar')
                    ...
                    @case('layout')
                        ...
                        @break
                    @case('sparkles')
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>
                        @break
                    @case('team')
                    ...
                    @case('file-text')
                        ...
                        @break
                    @case('activity')
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
                        @break
                    @case('trending-up')
    ...
    <div class="p-4 border-t border-border">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full bg-accent flex items-center justify-center text-sm font-bold text-white">
