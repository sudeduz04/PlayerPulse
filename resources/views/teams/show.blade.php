            ...
            <div>
                <div class="bg-surface-700 border border-border rounded-xl overflow-hidden">
                    <div class="px-6 py-4 border-b border-border">
                        <h2 class="text-lg font-semibold text-white">Antrenörler ({{ $team->coaches->count() }})</h2>
                    </div>
                    <div class="divide-y divide-border">
                        @forelse($team->coaches as $coach)
                            <div class="px-6 py-4 flex items-center justify-between">
                                ...
                                @if(auth()->user()->isRole('manager'))
                                    <form method="POST" action="{{ route('manager.teams.remove-coach', [$team->id, $coach->id]) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    {{-- Assign Coach Form (manager only) --}}
                    @if(auth()->user()->isRole('manager'))
                        <div class="px-6 py-4 border-t border-border">
                            <form method="POST" action="{{ route('manager.teams.assign-coach', $team->id) }}">
                                @csrf
