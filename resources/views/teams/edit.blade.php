<x-layouts.app title="Takım Düzenle">
    <div class="max-w-2xl">
        <div class="mb-6">
            <a href="{{ route($routePrefix . '.teams.show', $team->id) }}" class="text-gray-500 hover:text-gray-300 text-sm transition-colors mb-2 inline-block">&larr; Takıma Dön</a>
            <h1 class="text-3xl font-bold text-white">Takım Düzenle</h1>
            <p class="text-gray-500 text-sm mt-1">{{ $team->name }}</p>
        </div>

        <div class="bg-surface-700 border border-border rounded-xl p-6">
            <form method="POST" action="{{ route($routePrefix . '.teams.update', $team->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="age_category" id="age_category" required
                                ...
                                @foreach(['U13', 'U14', 'U15', 'U16', 'U17', 'U19', 'Senior'] as $cat)
                                    <option value="{{ $cat }}" {{ old('age_category', $team->age_category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full px-4 py-3 bg-surface-600 border border-border rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent text-sm">{{ old('description', $team->description) }}</textarea>
